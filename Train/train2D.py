...
class ModelTrain2D(object):
    ...
    def train(self):
        ...
        _, height, width, channels=X_train_r.shape
        ...
        # Model Training
        logger='training.log'
        weights_name='weights.{epoch:02d}-{val_dice_coef:.2f}.hdf5'
        params='res+alpha.log'
        ...
        logging.info('model= %s p= %s k1= %s k2= %s k3= %s nfilter= %s act= %s alpha= %s max dice val= %s train dice= %s epoch= %s ', str(i), str(self.gene[0]), str(self.gene[1]), str(self.gene[2]), str(self.gene[3]), str(self.gene[4]), str(self.gene[5]), str(self.gene[6]), str(max_dice_val), str(dice_train), str(max_index))
