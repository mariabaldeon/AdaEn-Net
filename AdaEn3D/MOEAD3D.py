...
from Train.train3D import val_convert_patch
import os
...
class MOEAD3D(object):
    ...
    def log_name(self, generation):
        self.location="SearchLogs/Results3D"
    ...
    def model_train_bp(self,generation, gene, logger, weights_name, params, indv):
        i=indv
        model= get_3DAda(h=self.patch_size[0],w=self.patch_size[1], p=gene[0],k1=gene[1],k2=gene[2], k3=gene[3], nfilter=gene[4],
        actvfc=gene[5], blocks=gene[7], slices=self.patch_size[2], channels=self.channels, add=gene[8])
        model.summary()
        alpha=gene[6]
    ...
    def search(self):
        ...
        model= get_3DAda(h=self.patch_size[0],w=self.patch_size[1],k1=int(np.min(self.ker_size1)),
                k2=int(np.min(self.ker_size2)), k3=int(np.min(self.ker_size3)),nfilter=int(np.min(self.num_filters)), 
                         blocks=int(np.min(self.blocks)), slices=self.patch_size[2], channels=self.channels )
        ...
        model=get_3DAda(h=self.patch_size[0],w=self.patch_size[1],k1=int(np.max(self.ker_size1)),
            k2=int(np.max(self.ker_size2)), k3=int(np.max(self.ker_size3)),nfilter=int(np.max(self.num_filters)),
                        blocks=int(np.max(self.blocks)), slices=self.patch_size[2], channels=self.channels )
