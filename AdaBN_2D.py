...
def last_blck(frst_block, previous_block, nfilter, k1, k2, k3, actvfc, p, add): 
    ...
    previous_block=UpSampling2D(size=(2,2))(previous_block)
    previous_block=Conv2D(filters=nfilter, kernel_size=(2,2), padding='same', activation=actvfc, 
    ...
    res= Add()([x1, x3])
    output= Conv2D(filters=1, kernel_size=1, activation='linear', kernel_initializer='he_uniform' )(res) 
...
def get_2DAda(h=128, w=128,p=0.5,k1=3,k2=3, k3=3, nfilter=32,actvfc='relu', blocks=9, channels=1, add=True):
    ...
    output= Conv2D(filters=1, kernel_size=1, activation='sigmoid', kernel_initializer='he_uniform' )(output)
