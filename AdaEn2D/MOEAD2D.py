...
from keras.callbacks import CSVLogger,ModelCheckpoint, EarlyStopping, TerminateOnNaN
...
def change_format(X_train_r, y_train_r, X_val_r, y_val_r):
    n, h, w, s, c=X_train_r.shape
    nval,_, _, sval,_=X_val_r.shape
    tot_train=n*s
    tot_val=nval*sval

    X_train=np.zeros((tot_train,h,w,c))
    y_train=np.zeros((tot_train,h,w,c))
    X_val=np.zeros((tot_val,h,w,c))
    y_val=np.zeros((tot_val,h,w,c))

    k=0
    for i in range(n):
        for j in range(s):
            X_train[k,:,:,:]=X_train_r[i,:,:,j,:]
            y_train[k,:,:,:]=y_train_r[i,:,:,j,:]
            k=k+1
    k=0
    for i in range(nval):
        for j in range(sval):
...
class MOEAD2D(object):
    ...
    def log_name(self, generation):
        self.location="SearchLogs/Results2D"
        ...
        for i in range(0,self.pop_size):
            name=self.location+"/"+str(generation)+'_'+str(i)+'_training.log'
            w_name=self.location+"/"+str(generation)+'_'+str(i)+'_weights.{epoch:02d}-{val_loss:.2f}.hdf5'
            p_name=self.location+"/"+str(generation)+'_'+str(i)+'_res+alpha.log'
            logger.append(name)
    ...
    def compute_neighbors(self, weight_vectors):
    ...
    def generate_parents(self, neighbor,individual_index, OF):
    ...
    def real_pareto(self, models_checked, Z_ref, max_OF):
    ...
    def model_train_bp(self,generation, gene, logger, weights_name, params, indv):
        i=indv
        model= get_2DAda(h=self.height,w=self.width, p=gene[0],k1=gene[1],k2=gene[2], k3=gene[3], nfilter=gene[4],actvfc=gene[5],
                       blocks=gene[7], channels=self.channels, add=gene[8])
        alpha=gene[6]
        ...
        early_stopper=EarlyStopping(monitor='val_loss', min_delta=0.01, patience=20, mode='auto')
        #Fit the model
    ...
    def search(self):
        ...
        _, self.height, self.width, self.channels=self.X_train_r.shape
        ...
        # Sort the initial population such that the architecture with the smallest total loss is with weights (0,1)
        models_checked=self.SortInitialPop(models_checked)

        #1.2. Calculate the reference point
        model= get_2DAda(h=self.height,w=self.width,k1=int(np.min(self.ker_size1)),k2=int(np.min(self.ker_size2)), k3=int(np.min(self.ker_size3)),
                       nfilter=int(np.min(self.num_filters)), blocks=int(np.min(self.blocks)), channels=self.channels )
        ...
        neighbor=self.compute_neighbors(weight_vectors)
        ...
        model=get_2DAda(h=self.height,w=self.width,k1=int(np.max(self.ker_size1)),k2=int(np.max(self.ker_size2)), k3=int(np.max(self.ker_size3)),
                       nfilter=int(np.max(self.num_filters)),blocks=int(np.max(self.blocks)), channels=self.channels )
        max_parameters=model.count_params()

        max_total_loss=1+1*self.w_tloss+self.w_eloss*1
        ...
        for i in range(start,self.max_iter):
            ...
            for j in range(self.pop_size):
                ...
                parent1, individual=self.generate_parents(neighbor,j, OF)
               ...
               #Calculate BI OF of each neighbor and compare with the child
                ...
                #OF.to_csv(self.location+'/OF.csv')
                models_checked.to_csv(self.location+'/models_checked.csv')
                pareto_solutions=self.real_pareto(models_checked, Z_ref, max_OF)
                pareto_solutions.to_csv(self.location+'/pareto_solutions.csv')
